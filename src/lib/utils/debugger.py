...
def get_time():
  ...
  data_times = f"{month}-{day}-{hour}-{minute}"
  return data_times
...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
               num_classes=-1, dataset=None, down_ratio=4, current_time=current_time):
    self.ipynb = ipynb
    if not self.ipynb:
    ...
    self.colors1 = np.random.randint(0, 256,size=(10,1,1,3), dtype=np.uint8)
    print(f"self.colors1: { self.colors1}")
    if self.theme == 'white':
  ...
  def add_coco_bbox(self, bbox, cat, conf=1, show_txt=False, img_id='default'): 
    ...
    cv2.rectangle(
      ...
      self.imgs[img_id], (bbox[0], bbox[1]), (bbox[2], bbox[3]), tuple(self.colors[cat][0]), 10) # -2
    print(tuple(self.colors[cat][0]))
    if show_txt:
  ...
  def add_4ps_coco_bbox(self, bbox, cat, conf, logi=None, show_txt=False, img_id='default'): 
    ...
    # print("img_id",img_id)
    # cat1 = np.random.randint(0,10)
    
    # color = tuple(np.array(self.colors[cat][0]).astype(int).tolist())
    # color = tuple(map(int, np.array(self.colors1[cat1][0]).flatten()))
    # color = tuple(self.colors1[cat][0].flatten())
    # print(f"cat1: {cat1} color: {color}")
    first_num = int(txt.split(",")[0])
    color = color_self.get(first_num, (128,128,128))
    cv2.line(self.imgs[img_id],(bbox[0],bbox[1]),(bbox[2],bbox[3]), color,2)
    cv2.line(self.imgs[img_id],(bbox[2],bbox[3]),(bbox[4],bbox[5]), color,2)
    cv2.line(self.imgs[img_id],(bbox[4],bbox[5]),(bbox[6],bbox[7]), color,2)
    cv2.line(self.imgs[img_id],(bbox[6],bbox[7]),(bbox[0],bbox[1]), color,2) # 1 - 5
    ...
    if not logi is None:
      ...
      #               (bbox[0] + cat_size[0], bbox[1] - 2), c, -1)
      # print(f"txt: {txt}")
      cv2.putText(self.imgs[img_id], txt, (bbox[0], bbox[1] + 10), 
                  font, 0.3, (0, 0, 255), thickness=1, lineType=cv2.LINE_AA) #1 - 5 # 0.20 _ 0.60
  ...
  def save_all_imgs(self, image_name, path='./cache/debug/', prefix='', genID=True):
    ...
    for i, v in self.imgs.items():
      ...
      folder_path = os.path.join("/ssdata/user/gangwei.li/Code/LORE-TSR/src/data","vis", self.current_time) 
      os.makedirs(folder_path, exist_ok=True)
      cv2.imwrite(folder_path + '/%s'%image_name, v)
