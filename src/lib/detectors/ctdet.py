...
from lib.external.shapelyNMS import pnms
...
class CtdetDetector(BaseDetector):
  ...
  def process(self, images, origin, return_time=False, batch=None):
    ...
    with torch.no_grad():
      ...
      output = outputs[-1]
      end_infer = time.time()
      print("model infer time",end_infer-start_infer)
      # print("batch",batch)
      ...
      print("model infer tensor change time",forward_time-end_infer)
      ...
      print("model infer corner_time time",corner_time-forward_time)
      # print("scores, inds, ys, xs, st_reg, corner_dict",scores, inds, ys, xs, st_reg, corner_dict)
      ...
      ps_decode_time = time.time()
      print("model infer psdecode time",ps_decode_time-corner_time)
      # corner_output = torch.cat((xs.permute(2,1,0),ys.permute(2,1,0),st_reg,scores),dim=2)
      #logi = self.process_logi(logi)
      decode_time = time.time()
    print("model infer decode time",decode_time-forward_time)
    if return_time:
  ...
  def show_results(self, debugger, image, results, corner, image_name, logi=None):
    ...
    count = 0
    import os
    fc = open(os.path.join(self.opt.output_dir,image_name+'center.txt'),'w') #bounding boxes saved
    fv = open(os.path.join(self.opt.output_dir,image_name+'corner.txt'),'w')
    fl = open(os.path.join(self.opt.output_dir,image_name+'log.txt'),'w') #logic axis saved 
    for j in range(1, self.num_classes + 1):
      k = 0
      # print("logi",len(logi))
      # print("len(results[j])",len(results[j]))
