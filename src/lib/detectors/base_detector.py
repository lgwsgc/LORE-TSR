...
class BaseDetector(object):
  ...
  def run(self, opt, image_or_path_or_tensor, image_anno=None, meta=None):
    # print(opt)
    # input_data = torch.randn(1, 3, 1024, 1024).cuda()
    # model_path = './wired_model.onnx'
    # torch.onnx.export(self.model, input_data, model_path, verbose=False,input_names=["input"],opset_version=16)
    ...
    for scale in self.scales:
      ...
      if self.opt.wiz_detect:
        print("self.opt.wiz_detect",self.opt.wiz_detect)
      ...
      time3 = time.time()
      raw_dets = dets
      print("model all infer time:",time3-time2)
      torch.cuda.synchronize()
