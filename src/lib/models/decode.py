...
def ctdet_4ps_decode(heat, wh, ax, cr, corner_dict=None, reg=None, cat_spec_wh=False, K=100, wiz_rev = False):
    ...
    print("rev_time_s1",rev_time_s1-start_time_topk)
    if wiz_rev :
        bboxes_rev = bboxes.clone()
        bboxes_cpu = bboxes.clone().cpu().numpy()

        gboxes = corner_dict['gboxes']
        gboxes_cpu = gboxes.cpu().numpy()
        ...
        for i in range(num_bboxes):
            if scores[0,i,0] >= 0.3 :
                    ...
                    if corner_scores[0,j,0] >= 0.3:
                        ...
                        if is_group_faster_faster(bbox, gbox): 
                            ...
                            if bboxes_rev[0, i, 2*ind4ps] == bboxes[0, i, 2*ind4ps] and bboxes_rev[0, i, 2*ind4ps+1] == bboxes[0, i, 2*ind4ps+1]:
                                ...
                                # time_for_3 = time.time()
                                # print("3",time_for_3-time_for_2)
                            else:
                                ...
                                # time_for_4 = time.time()
                                # print("4",time_for_4-time_for_3)
                                if dist(origin_x, origin_y, old_x, old_y) >= dist(origin_x, origin_y, cr_x, cr_y):
                                    ...
                                    continue
                                # time_for_5 = time.time()
                                # print("5",time_for_5-time_for_4)
                        else:
                ...
                break
    rev_time_s2 = time.time()
    print("rev_time_s2",rev_time_s2-rev_time_s1)
    if wiz_rev:
    ...
    rev_time_s3 = time.time()
    print("rev_time_s3",rev_time_s3-rev_time_s2)
